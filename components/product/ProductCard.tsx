  ...
  return (
    <article className="group bg-white rounded-2xl shadow-card overflow-hidden flex flex-col border border-brand-cream-dark hover:shadow-premium hover:border-brand-gold/30 transition-all duration-300">
      ...
      <div className="p-6 flex flex-col flex-1">
        <p className="text-xs text-brand-gold font-semibold mb-2">{product.routineLabel}</p>
        <Link href={`/products/${product.slug}`}>
          <h3 className="font-extrabold text-lg leading-snug text-brand-plum group-hover:text-brand-plum-light transition">
            {product.cardTitleAr}
          </h3>
          <p className="text-sm text-[#6B6B6B] mt-3 leading-relaxed line-clamp-4">
        ...
        <div className="mt-4 flex items-baseline gap-2">
          ...
          <span className="font-extrabold text-xl text-brand-plum">199 ر.س</span>
        </div>
        <p className="text-xs text-brand-trust mt-1">2 بـ 279 · 3 بـ 349 · COD</p>
        <Link href={`/products/${product.slug}`} className="mt-5">
          <Button fullWidth className="bg-brand-plum hover:bg-brand-plum-light">
            اطلبي الآن
          </Button>
        </Link>
      </div>
  ...
  return (
    <div className="flex gap-3 p-4 bg-white rounded-xl border border-brand-cream-dark shadow-sm">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-[#6B6B6B] truncate">{product.problemLineAr}</p>
        <p className="text-sm text-brand-plum font-bold mt-1">199 ريال</p>
      </div>
